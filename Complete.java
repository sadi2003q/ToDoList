...
public class Complete {
    ...
    public void ChangeTSchedule(Event e) throws IOException {
    ...
    public static EventHandler<MouseEvent> redCircleHandler(HBox hbox, AnchorPane main, Complete controller, FlowPane flowPane, double currentPosition){
        return mouseEvent -> {
            ...
            for(Node node: list){
                if(node instanceof FlowPane){
                    ...
                    for(Node newNode : pane){
                        if(newNode instanceof Text){
                            //System.out.println("this file "+((Text) newNode).getText());
                            String[] line=(((Text) newNode).getText()).split("\n");
                            try {
                                new fileWriter().deleteTask2(line[0]);
                            } catch (IOException e) {
    ...
    private void loadTasksFromFileAndShow() {
        try {
            ArrayList<ArrayList<String>> taskInfoList = fileReader.readTasksFromFile2();
